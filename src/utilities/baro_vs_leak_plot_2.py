...
output_file_path = "/people/ajay148/Desktop/plots_p/baro_vs_leak_plots"

# --- Simulation settings ---
pre_time = 15.6
step_size = 0.01
period = 900.0  # total 15 minutes
...
y = sim_object.get_results([
    ...
    'heart/q_lv', 
    'A_aortic_arch_C_1/u', 'common_carotida_L_baro/P_baro',
    'common_carotida_L_baro/f_ab'
])
...
# --- Plotting ---
fig, axs = plt.subplots(3, 2, figsize=(12, 8), sharex=False, sharey=False)
...
axs[1,0].set_ylim(bottom=0,top=500)
...
# P_baro
axs[2,0].plot(t, y[4][0] * conversion_mmHg, color='purple')
axs[2,0].set_ylabel('pressure [mmHg]')
axs[2,0].set_xlim([0, t[-1]])
axs[2,0].set_ylim(bottom=0)
axs[2,0].set_title('baroreceptor')

# f_ab_baro
axs[2,1].plot(t, y[5][0], color='purple')
axs[2,1].set_ylabel('firing rate [s^{-1}]')
axs[2,1].set_xlim([0, t[-1]])
axs[2,1].set_ylim(bottom=0)
axs[2,1].set_title('baroreceptor')
